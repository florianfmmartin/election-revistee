...
type Winners = [string, number][];

const res = resultat.circonscriptions.map((cr) =>
  cr.candidats.map((cd) => ({
...
const getBestCandidat = (candidats: Candidat[]) =>
  candidats.sort((cd1, cd2) => cd1.vote < cd2.vote ? 1 : -1)[0];

const getBestInEach = (data: Resultat) => data.map(getBestCandidat);

const compileBests = (bestInCrs: Candidat[]): Winners =>
  Object.entries(
...
const getResult = (data: Resultat) => compileBests(getBestInEach(data));
...
const bigWinner = (winners: Winners, coalition: boolean) => {
  let winnersCalcul: any = winners;
